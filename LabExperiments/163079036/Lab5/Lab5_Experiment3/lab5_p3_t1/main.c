...
void glcd_DisplayImage_Ref(char *Image_Array_Location)
{
    unsigned char page_num = 0;
    unsigned char column = 0;
    //uint32_t Image_Array_Index = 0;
    ...
    for(page_num = 0; page_num < 8; page_num++)
    {
        ...
        for (column=0; column<128; column++)
        {
            glcd_setcolumn(column);
            //char Image_Array_Value = &Image_Array_Location;
            glcd_data(*Image_Array_Location);
            Image_Array_Location++;
        }
        column=0;
    }
}
...
void set_refresh_delay(void)
{

    if (JoyStick_Vertical_ADC_Value <200)
    {
        if(Delay_Cheetah <110000)
            {
            Delay_Cheetah = 6700;
            }
        else
            {
            Delay_Cheetah = Delay_Cheetah - 100000;
            }
    }
    if (JoyStick_Vertical_ADC_Value >3800)
    {
        Delay_Cheetah = Delay_Cheetah + 100000;
    }
